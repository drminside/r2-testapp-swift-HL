...
extension AppDelegate {
    ...
    internal func addPublicationToLibrary(url: URL, needUIUpdate:Bool) -> Bool {
        ...
        case "lcpl":
            ...
            lcpService.importPublication(from: documentsUrl) { publication, error in
                if FileManager.default.fileExists(atPath: documentsUrl.path) {
                    try? FileManager.default.removeItem(at: documentsUrl)
                }
                guard let (localURL, downloadTask) = publication else {
                    if let error = error {
                ...
                if self.lightParsePublication(at: Location(absolutePath: localURL.path, relativePath: "", type: .epub)) {
...
extension AppDelegate: LibraryViewControllerDelegate {
    ...
    func loadPublication(withId id: String?, completion: @escaping (Drm?, Error?) -> Void) throws {
        ...
        switch drm.brand {
        case .lcp:
            lcpService.retrieveLicense(at: publicationPath) { license, error in
                var drm = drm
                drm.license = license
                try? parsingCallback(drm)
                completion(drm, error)
            }
        }
        #endif
    }
